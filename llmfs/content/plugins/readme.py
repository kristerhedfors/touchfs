...
class ReadmeGenerator(ProcPlugin):
    ...
    def _get_llmfs_overview(self, fs_structure: Dict[str, Union[Dict, FileNode]]) -> str:
        ...
        node = fs_structure[llmfs_dir]
        llmfs_files = self._get_node_attr(node, 'children', {})
        
        overview.append(f"{HEADER}## .llmfs Directory Overview{RESET}\n")
        overview.append(f"The .llmfs directory contains special files that control and monitor the filesystem:\n")
...
{SUBHEADER}4. **Context System**{RESET}
   - Use llmfs_context to analyze project context:
     {CODE}```bash
...
{SUBHEADER}2. **Command Line Tool (llmfs_context)**{RESET}
